...
public:
	...
	typedef union {TDatum *pD; T* pT;} conv_union;
	typedef union {const TDatum *pD; const T* pT;} const_conv_union;
...
#else
		const_conv_union u;	u.pD = m_data;
		return u.pT[i];
#endif
	}
  ...
  ILINE const T& operator[](unsigned int i) const 
	...
	ILINE void push_back () 
	{
		...
		{
			CryLogAlways("CryFixedArray::push_back() failing as array is full - NOT adding element");
			CRY_ASSERT_MESSAGE(0, "CryFixedArray::push_back() failing as array is full - NOT adding element");
		}
	}
...
public:
	...
	ILINE bool removeAt(uint32 i)
	{
		...
		{
			CryLogAlways("CryFixedArray::removeAt() failed as i=%d is out of range of curSize=%d", i, m_curSize[0]);
