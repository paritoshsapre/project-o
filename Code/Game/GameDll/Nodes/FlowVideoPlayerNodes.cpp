...
public:
	...
	virtual ~CFlowFlashVideoPlayerNode()
	{
		...
		m_pActionMapMan->RemoveExtraActionListener(this);
	}
	...
	virtual void ProcessEvent( EFlowEvent event, SActivationInfo *pActInfo )
	{
		...
		case eFE_Initialize:
			...
			if (gEnv->pFlashUI)
			{
				...
				if (m_pElement)
				{
					m_pElement->AddEventListener(this);
				}
				else
				{
	...
	{
		if (event.sDisplayName == "OnPlay")
		{
			ActivateOutput(&m_ActInfo, eO_OnPlay, true);
		}
		else if (event.sDisplayName == "OnPause")
		{
			ActivateOutput(&m_ActInfo, eO_OnPause, true);
		}
		else if (event.sDisplayName == "OnResume")
		{
			ActivateOutput(&m_ActInfo, eO_OnResume, true);
		}
		else if (event.sDisplayName == "OnStop")
		{
		...
		else if (event.sDisplayName == "OnLooped")
		{
			ActivateOutput(&m_ActInfo, eO_OnLooped, true);
		}
		else if (event.sDisplayName == "OnFileNotFound")
		{
			UnloadPlayerNextFrame();
