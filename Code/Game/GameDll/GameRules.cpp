...
#define NOTIFY_UI( fct ) { CUIManager::GetInstance()->GetUIObjectives()->fct; }
...
void CGameRules::AddObjective(int teamId, const char *objective, EMissionObjectiveState status, EntityId entityId)
{
	...
	if ( m_pGameFramework->GetClientActor() )
	{
		...
		if( pActor && GetTeam(pActor->GetEntityId()) == teamId )
			NOTIFY_UI( MissionObjectiveAdded( objective, status ) );
	}
}
...
void CGameRules::SetObjectiveStatus(int teamId, const char *objective, EMissionObjectiveState status)
{
	...
	if ( m_pGameFramework->GetClientActor() )
	{
		CActor* pActor = GetActorByChannelId(m_pGameFramework->GetClientActor()->GetChannelId());
		if( pActor && GetTeam(pActor->GetEntityId()) == teamId )
			NOTIFY_UI( MissionObjectiveStateChanged( objective, status ) );
	}
}
...
void CGameRules::RemoveObjective(int teamId, const char *objective)
{
	...
	if ( m_pGameFramework->GetClientActor() )
	{
		CActor* pActor = GetActorByChannelId(m_pGameFramework->GetClientActor()->GetChannelId());
		if( pActor && GetTeam(pActor->GetEntityId()) == teamId )
			NOTIFY_UI( MissionObjectiveRemoved( objective ) );
	}
}
...
void CGameRules::ResetObjectives()
{
	...
	NOTIFY_UI( MissionObjectivesReset() );
}
