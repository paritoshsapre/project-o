      ...
      if(bigObject) then
        if (damage > 0.5) then 
				...
				if (collider and not collider.actor) then
				  ...
				  if(contactVelocity > killVelocity and contactVelocityCollider > killVelocity and colliderMass > 50 and not entity.actor:IsPlayer()) then  				  	
				  	...
				  	-- don't allow killing friendly AIs by collisions
						if((not AI) or (not AI.Hostile(entity.id, g_localActorId, false))) then
							return;
						end
				  	...
				  	if(bFall) then
						...
						if(g_localActorId and AI and AI.Hostile(entity.id, g_localActorId, false)) then
			...
			if (damage >= 0.5) then				  				
				...
				if(entity.actor and not self:IsMultiplayer() and not (AI and AI.Hostile(entity.id, g_localActorId, false))) then
					if(entity.id ~= g_localActorId) then
		...
		if(target.Properties.Damage.bNoDeath and target.Properties.Damage.bNoDeath==1) then
			...
			if(target.id == g_localActorId) then
				...
				if(hit.shooterId == g_localActorId) then
					if((not AI) or (not AI.Hostile(hit.target.id, g_localActorId, false))) then
						target.actor:Fall(hit.pos);
						return false;
					end
				end
...
function SinglePlayer:ProcessDeath(hit)
	...
	if(hit.target.id == g_localActorId) then
		...
		if(hit.shooterId == g_localActorId) then 
			if((not AI) or (not AI.Hostile(hit.target.id, g_localActorId, false))) then -- should never happen atm, because we prevent death when player shot friend
