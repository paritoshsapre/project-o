...
_Node_alloc_obj* node_alloc<_alloc,__threads, _Size>::_S_refill(size_t __n) {
	...
	union {int *pI; long volatile *pL;} u;	u.pI = &__pCurrentHugeBlock->_M_count;
	Node_Alloc_Lock<__threads, _Size>::ATOMIC_EXCHANGE_ADD(u.pL, __nobjs);
																													...
																													int& __nobjs) 
{
	...
	_Mem_block *__pblock;
	union {_Mem_block* volatile *pMB; long volatile *pL;} un;
	un.pMB = &_S_free_mem_blocks;
	long volatile* pMemBlock = un.pL;
...
#define _ALLOCATION_SIZE 512 * 1024
	...
	union {_Node_alloc_Mem_block_Huge * volatile* pNMH; long volatile *pL;} u;
	do {
		__old_block = __pCurrentHugeBlock;
		u.pNMH = &__pCurrentHugeBlock;
	} while (!Node_Alloc_Lock<__threads, _Size>::ATOMIC_CAS(u.pL, (long)__pBlock, (long)__old_block));
...
//enum {_ALIGNMENT = 16, __MAX_BYTES = 512};
//#else
////enum {_ALIGNMENT = 8, _ALIGN_SHIFT = 3, __MAX_BYTES = 128};
//enum {_ALIGNMENT = 8, __MAX_BYTES = 256};
//#endif /* __OS400__ */
//
//
...
//	typedef char value_type;
//	/* __n must be > 0      */
//	static void *  allocate(size_t __n, size_t __align = _ALIGNMENT) 
//	{ 
//		if (__n > (size_t)__MAX_BYTES) 
//		{
//			Node_Allocator<_alloc> all;
...
//	}
//
//	static void *  alloc(size_t __n, size_t __align = _ALIGNMENT) 
//	{ 
//		return allocate(__n, __align);
//	}
//
//	/* __p may not be 0 */
//	static void  deallocate(void *__p, size_t __n, size_t __align = _ALIGNMENT) 
//	{
//		if (__n > (size_t)__MAX_BYTES) 
//		{
//			Node_Allocator<_alloc> all;
...
//	}
//	static void  dealloc(void *__p, size_t __n, size_t __align = _ALIGNMENT) 
//	{
