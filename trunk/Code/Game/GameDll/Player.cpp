...
void CPlayer::Update(SEntityUpdateContext& ctx, int updateSlot)
{
	...
	// for ded server: fake a view update
...
//						m_inertia = g_pGameCVars->pl_clientInertia;
			            ...
			            params = GetAnimatedCharacter()->GetParams();
            			params.inertia = g_pGameCVars->pl_clientInertia;
			            SetAnimatedCharacterParams( params );
					}
...
void CPlayer::SetIK( const SActorFrameMovementParams& frameMovementParams )
{
	...
	{
		// Normal case
		m_lookAim.UpdateLook( this, pCharacter, lookEnabled, GetLookFOV(m_params), frameMovementParams.lookTarget, lookIKLayer );
	}
	else
	{
		...
		m_lookAim.UpdateLook( this, pCharacter, true, GetLookFOV(m_params), target, lookIKLayer, lookIKBlends );
	}
...
void CPlayer::UpdateView(SViewParams &viewParams)
{
	...
	if(g_pGameCVars->cl_cam_newsystem != 0 && IsThirdPerson())
	{
		// TODO!
...
//			SetHealth(GetHealth() - max(1.0f, 30.0f * frameTime));
	...
	if (isClient)
	{
		...
		if(IListener* pListener = gEnv->pSoundSystem->GetListener(LISTENERID_STANDARD))
...
void CPlayer::PlaySound(EPlayerSounds sound, bool play, bool param /*= false*/, const char* paramName /*=NULL*/, float paramValue /*=0.0f*/)
{
	...
	case ESound_Fall_Drop:
		soundName = "sounds/physics:foleys/player:melee_push";
		soundSemantic = eSoundSemantic_Player_Foley_Voice;
...
bool CPlayer::UpdateLadderAnimation(ELadderState eLS, ELadderDirection eLDIR, float time /*=0.0f*/)
{
	switch(eLS)
	{
			...
			case eLS_Climb:
				...
				m_pAnimatedCharacter->GetAnimationGraphState()->SetInput("Action","climbLadder");
