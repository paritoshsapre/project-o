...
CUIInput::CUIInput()
{
	...
	m_pUIEvents = pFlashUI->CreateEventSystem( "System", IUIEventSystem::eEST_UI_TO_SYSTEM );
...
{
	if ( activationMode == eAAM_OnRelease && !gEnv->IsEditor() && g_pGame->GetIGameFramework()->IsGameStarted() )
	{
...
#define SEND_CONTROLLER_EVENT(evt) 	static IFlashUIPtr pFlashUI = GetIFlashUIPtr(); \
	...
	{ \
		...
		{ \
			case eAAM_OnPress: 	pFlashUI->DispatchControllerEvent( IUIElement::evt, IUIElement::eCIS_OnPress ); break; \
			case eAAM_OnRelease: pFlashUI->DispatchControllerEvent( IUIElement::evt, IUIElement::eCIS_OnRelease ); break;\
		} \
	}
