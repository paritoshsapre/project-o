...
void CRocketLauncher::ActivateLaserDot(bool activate, bool fp)
{
	if(activate)
	{
		IParticleEffect * pEffect = gEnv->pParticleManager->FindEffect(fp?"muzzleflash.LAM.aimdot_FP":"muzzleflash.LAM.aimdot");
...
void CRocketLauncher::PostDropAnim()
{
	...
	if(m_fm && GetAmmoCount(m_fm->GetAmmoType())<=0)
	{
		...
		if(m_smokeEffectSlot==-1)
		{
			IParticleEffect * pEffect = gEnv->pParticleManager->FindEffect("weapon_fx.LAW.empty");
